...
from .utils.timm_loading import _load_weights as timm_load_weights
...
class Attention(nn.Module):
    def __init__(
        ...
        rel_sp_h = 2 * window_size[0] - 1
        rel_sp_w = 2 * window_size[1] - 1
        
        self.rel_pos_h = nn.Parameter(torch.zeros(rel_sp_h, head_dim))
        self.rel_pos_w = nn.Parameter(torch.zeros(rel_sp_w, head_dim))
    ...
    def forward(self, x, H, W, rel_pos_bias=None):
        ...
        attn = calc_rel_pos_spatial(attn, q, self.window_size, 
            self.window_size, self.rel_pos_h, self.rel_pos_w)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., init_values=None, act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_checkpoint=False,
...
class ViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 out_indices=[11], interval=3, init_cfg=None, mask_cfg=None):
        super().__init__()
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed.grid_size = self.patch_embed.patch_shape
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, use_checkpoint=use_checkpoint,
    ...
    def init_weights(self):
        ...
        if self.init_cfg['type'] == 'timm_Pretrained':
            checkpoint_model = torch.load(self.init_cfg['checkpoint'], map_location='cpu')
            state_dict = self.state_dict()
            ckp_dict = {k:v for k,v in checkpoint_model.items() if k in state_dict.keys()}
            # interpolate position embedding
            interpolate_pos_embed(self, ckp_dict)
            # modify patch_embed shape for conv-based patchfy
            O, I, H, W = state_dict['patch_embed.proj.weight'].shape
            ckp_dict['patch_embed.proj.weight'] = ckp_dict['patch_embed.proj.weight'].reshape(O, -1, H, W)
            # load pre-trained model
            msg = self.load_state_dict(ckp_dict, strict=False)
            logger.info(msg)
        
        elif self.init_cfg['type'] == 'pretrained':
            checkpoint_model = torch.load(self.init_cfg['checkpoint'], map_location='cpu')
            logger.info("Load pre-trained checkpoint from: %s" % self.init_cfg['checkpoint'])
            ckp_dict = checkpoint_model['model']
            state_dict = self.state_dict()
        ...
        elif self.init_cfg['type'] == 'timm':
            timm_load_weights(self, self.init_cfg['checkpoint'], logger=logger)
        
        elif self.init_cfg['type'] == 'BEV-MAE':
            # for bev-mae pretrain backbone
            # only get the img_backbone part
            checkpoint_model = torch.load(self.init_cfg['checkpoint'], map_location='cpu')
            logger.info("Load pre-trained checkpoint from: %s" % self.init_cfg['checkpoint'])
            ckp_dict = checkpoint_model['state_dict']
            online_bkb_dict = {k:v for k,v in ckp_dict.items() if 'online_model.img_backbone' in k}
            online_bkb_dict = {k[len('online_model.img_backbone.'):]:v for k,v in online_bkb_dict.items() }
            online_bkb_dict = {k:v for k,v in online_bkb_dict.items() if k !='mask_token'}
            state_dict = self.state_dict()
            # load pre-trained model
            msg = self.load_state_dict(online_bkb_dict, strict=True)
            logger.info(msg)
        else:
