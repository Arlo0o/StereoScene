...
class SpconvNeck3D(BaseModule):
    ...
    def __init__(self,
        ...
        super(SpconvNeck3D, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, inputs):
        ...
        for i in range(1, self.num_out):
            out_shape = laterals[i].shape[2:]
            laterals[i] = laterals[i] + F.interpolate(laterals[i - 1], size=out_shape, align_corners=False, **self.upsample_cfg)
